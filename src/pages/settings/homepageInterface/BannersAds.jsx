...
export default function BannersAds() {
  ...
  const onSubmit = (value) => {
    ...
    if (!selectedBanner) createBanner({ ...value, image_url: image }, onSuccess, onFail)
    else updateBanner(selectedBanner.id, { ...value, image_url: image }, onSuccess, onFail)
  }
  ...
  return (
    <div>
      <div className='flex justify-between items-center'>
        <p className='my-4 font-semibold text-[20px]'>Danh sách Banners quảng cáo</p>
        <Button
          type='primary'
          onClick={() => {
            setShowModal(true)
