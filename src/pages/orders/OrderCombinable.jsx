...
const OrderCombinable = ({data, popOverContent, dataOrderDetail, isOpenModal}) => {
    const shopId = getPathByIndex(2)
    const { confirmCombine } = useShopsOrder(state => state)
    const [dataCombine, setDataCombine] = useState(data.pre_combine_pkg_list)
    const handleGetPkgIdSubmit = (PkgId) => { console.log("PkgId: ", PkgId)}
            ...
            return (
                {
                    ...
                    order_id_list: item.pre_combine_pkg_id === CombinePkgId
    ...
    const columnsOrderCombine = [
        ...
        },
        {
            ...
            align: "center",
            render: (_, record) => (
            <Popconfirm title="Bạn có chắc chắn muốn xoá order này?" onConfirm={() => handleRemoveCombineItem(record.pre_combine_pkg_id, record.order_id)}>
                <a>Delete</a>
            </Popconfirm>
            )
        }
    ...
    return (
        <>
            {dataCombine?.map((item, index) => {
                const orderCombine = item.order_id_list.map(orderItem => (
                    dataOrderDetail?.filter(orderTableItem => orderTableItem.order_id === orderItem)
                )).flat()
                ...
                return (
                    <div key={index} className="mb-10">
                        <Table
                            columns={columnsOrderCombine}
                            dataSource={orderCombineConvert}
                            bordered
                            pagination={false}
                        />
            ...
            <div className="flex flex-wrap items-center justify-end gap-10">
                <Button type="primary" htmlType="submit" onClick={handleCombineSubmit}>Submit</Button>
            </div>
        </>
    )
}
