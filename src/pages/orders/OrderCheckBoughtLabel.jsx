        ...
        <div className="p-10">
            ...
            <Form onFinish={onSearch} onFinishFailed={() => { }} className='md:w-[400px] relative border-[1px] border-solid border-[#d9d9d9] rounded-[6px] pr-[90px]'>
                    <Form.Item name="package_id" className="mb-0">
                        <Input placeholder='Tìm kiếm theo Package ID...' className="!border-none focus:shadow-none bg-transparent rounded-none"/>
                    </Form.Item>
            ...
            {labelSearch.length > 0 &&
                <div className='mt-5'>
                    {labelSearch.map(label => (
                        <Card title={`Thông tin tìm kiếm  cho [ ${label.replace('.pdf', '')} ]`} 
                            className="mb-3"
                            extra={<DownloadOutlined onClick={() => handleDownloadFile(label)} className="text-[20px] cursor-pointer" />}
                        >
                            <p><span className="min-w-[70px] inline-block font-semibold">Package ID:</span> {label.replace('.pdf', '')}</p>
                            <p>
